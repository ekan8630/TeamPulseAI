...
export default function Signup() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      pushToast({ type: 'error', title: 'Signup error', message: msg })
    } finally {
      setLoading(false)
    }
  }
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 13, opacity: 0.8, marginTop: 2 }}>Start organizing tasks by team role</div>
          </div>
        ...
        <form onSubmit={onSubmit} style={{ marginTop: 16, display: 'grid', gap: 12 }}>
          <label style={{ display: 'grid', gap: 6 }}>
            <div style={{ fontSize: 13, opacity: 0.9 }}>Full name</div>
            <input
              value={name}
